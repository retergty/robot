...
for t=sample_time:sample_time:StepTime
    if(t<=StartTime)
        ...
        Ref_ZMPy(1,i) = ZMPy_StepLength/2;
    elseif(t<=StartTime + DblTime)
        ax = smothstep(ZMPx_StepLength,2*ZMPx_StepLength,DblTime);
        ay = smothstep(ZMPy_StepLength/2,-ZMPy_StepLength/2,DblTime);
        tmp_time = t-StartTime;
        Ref_ZMPx(1,i) = ax(1) + ax(2)*tmp_time + ax(3)*tmp_time^2 + ax(4)*tmp_time^3 + ax(5)*tmp_time^4;
        Ref_ZMPy(1,i) = ay(1) + ay(2)*tmp_time + ay(3)*tmp_time^2 + ay(4)*tmp_time^3 + ay(5)*tmp_time^4;
    else
        Ref_ZMPx(1,i) = 2*ZMPx_StepLength;
        Ref_ZMPy(1,i) = -ZMPy_StepLength/2;
    end
    i=i+1;
end
for t=sample_time:sample_time:StepTime
    if(t<=StartTime)
        ...
        Ref_ZMPy(1,i) = -ZMPy_StepLength/2;
    elseif(t<=StartTime + DblTime)
        ax = smothstep(2*ZMPx_StepLength,3*ZMPx_StepLength,DblTime);
        ay = smothstep(-ZMPy_StepLength/2,ZMPy_StepLength/2,DblTime);
        tmp_time = t-StartTime;
        Ref_ZMPx(1,i) = ax(1) + ax(2)*tmp_time + ax(3)*tmp_time^2 + ax(4)*tmp_time^3 + ax(5)*tmp_time^4;
        Ref_ZMPy(1,i) = ay(1) + ay(2)*tmp_time + ay(3)*tmp_time^2 + ay(4)*tmp_time^3 + ay(5)*tmp_time^4;
    else
        Ref_ZMPx(1,i) = 3*ZMPx_StepLength;
        Ref_ZMPy(1,i) = ZMPy_StepLength/2;
    end
    i=i+1;
end
...
for j=i:COUNT
    Ref_ZMPx(1,j) = 3*ZMPx_StepLength;
    Ref_ZMPy(1,j) = 0;
end
...
fid = fopen('preview_test2.txt','w');
fprintf(fid,"#pragma once \r\n");
fprintf(fid,"constexpr double TEST2_REFERENCE_ZMPX[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_ZMPY[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_INPUTX[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_INPUTY[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEX1[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEX2[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEX3[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEY1[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEY2[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEY3[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEZMPX[] = ");
fprintf(fid,"{");
for i=1:COUNT
...
fprintf(fid,"constexpr double TEST2_REFERENCE_STATEZMPY[] = ");
fprintf(fid,"{");
for i=1:COUNT
