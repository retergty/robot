...
template <typename scalar, size_t dimens, typename timeType>
class BaseMethod
{
public:
  BaseMethod(
      ...
      timeType time_total) : _initial(init), _target(target), _time_total(time_total){};

  virtual ~BaseMethod(){} : virtual Eigen::Vector<scalar, dimens> NowVal(const timeType now_time) = 0
  {
    return _target;
  }
  protected: 
  ...
  timeType _time_total;
};

template<typename scalar,typename timeType>
scalar BaseMethod<scalar,1,timeType>::virtual NowVal(const timeType now_time) = 0 
{
  return _target(0);
}

// t=0,t=T/2,t=T,t=0,t=T
template <typename scalar, size_t dimens, typename timeType, typename calscalar = scalar>
class FourPolyMethod final : public BaseMethod<scalar, dimens, timeType>
{
public:
  FourPolyMethod(
      const Eigen::Vector<scalar, dimens> &init,
      const Eigen::Vector<scalar, dimens> &target,
      timeType time_total,
      const Eigen::Vector<scalar, 5>[dimens](&b)) : BaseMethod(init, target, time_total)
  {
    calscalar T = time_total;
    Eigen::Matrix<scalar, 5, 5> A{
        {1, 0, 0, 0, 0},
        {1, T / 2, T * T / 4, T * T * T / 8, T * T * T * T / 16},
        {1, T, T * T, T * T * T, T * T * T * T},
    ...
    {
      a[i] = A.colPivHouseholderQr().solve(b[i]);
    }
  };

  Eigen::Vector<scalar, dimens> NowVal(const timeType now_time) override
  {
    Eigen::Vector<scalar,dimens> ret;
    calscalar t = static_cast<calscalar>(now_time);
    for(size_t i=0;i<dimens;++i){
      ret(i) =  a[i](0) + a[i](1) * t + a[i](2) * t * t + a[i](3) * t * t * t + a[i](4) * t * t * t * t;
    }
    return ret;
  }

private:
  Eigen::Vector<calscalar, 5>[dimens] a;
};
